...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    for i in range(num_patient_files):      
        ...
        # Create images
        patient_imgs = preprocess(current_patient_data, current_recordings)  
    ...
    #print("All data has been processed... saving for future use!")
    #hickle.dump(data_dict, "preprocessed_data.hickle", mode='w') 
    #data_dict = list(hickle.load('preprocessed_data.hickle').values())
    #data_dict = [(d['patient_imgs'], d['label'].argmax()) for i, d in data_dict.items()]
...
def preprocess(data, recordings): 
     location_order = ['AV', 'PV', 'TV', 'MV','Phc']     
     ...
     patient_imgs = torch.zeros(5, 224, 224)
     ...
     for j in range(len(recordings)):  
         rec = recordings[j]   
         g = Cinc_Graphs()
         img = g.make_tde2(rec, fs) 
         patient_imgs[location_indx[j]] = img 
     return patient_imgs

def collate_test(batch):
    patient_imgs = batch 
    outputs = {"patient_imgs": patient_imgs, "labels": torch.zeros(5).long()}
    return tuple(outputs.values())
...
def main(): 
    """ TRAIN """  
    #data_folder = '../data/physionet.org/files/circor-heart-sound/1.0.3/training_data'   
    #model_folder = 'models'
    #train_challenge_model(data_folder, model_folder, verbose=1)
    
    """ PRE-TRAINING """ 
    #pre_data_folder = '../data/pre_train/training-a' 
    #os.chdir(pre_data_folder) 
    # OLD DATA only has signal and label. No location.  
    #wav = 'a0001.wav' 
    #dat = 'a0001.dat'
    #hea ='a0001'
    #print("WAV",load_wav_file(wav)) 
    #print("DATA", load_patient_data(dat)) 
    #print("HEA", wfdb.rdrecord(hea).__dict__) 
    #verbose=0 
